...
OUTNAME          := circle-${RELEASE_VER}-docker
...
TYPE=container
CONTAINER_NAME = circle-$(MAJOR)-$(TYPE)-$(RELEASE_VER).$(BUILDDATE).$(ARCH)
...
$(KICKSTART_DIR):
	git clone --branch c$(MAJOR) --single-branch https://github.com/circle-linux/kickstarts.git kickstarts
