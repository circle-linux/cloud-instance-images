...
TYPE=Container
VARIANT=Base
CONTAINER_NAME = Circle-$(MAJOR)-$(TYPE)-$(VARIANT)-$(RELEASE_VER).$(BUILDDATE).$(ARCH)
KICKSTART_DIR  = kickstarts
KICKSTART_PATH = "${KICKSTART_DIR}/Circle-9-${TYPE}-${VARIANT}.ks"

OUTNAME          := circle-${RELEASE_VER}-${TYPE}-${VARIANT}
